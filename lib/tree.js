 ...
 * @Last Modified time: 2017-06-22 00:05:27
 */
...
class Tree {
  ...
  constructor(data) {
    const values = validate(
      ...
      joi.object().keys({
        ...
        key: joi
          .alternatives([joi.string(), joi.number().integer()])
          .required(),
        env: joi
          .alternatives([joi.string(), joi.number().integer()])
          .required(),
        ...
        itemType: joi
          .object()
          .valid([Array, Object, String, Number])
          .allow(null)
          .default(null),
        itemLength: joi.number().integer().allow(null).default(null),
      }),
    );
  ...
  /**
   * Print the tree recursively with key and corresponding env
   *
   * @param {number} [indent=0]
   *
   * @returns {string} - the representation of tree
   * @memberof Tree
   */
  print(indent = 0) {
